...
### 后台管理-博客内容信息扩展-作者-添加时间-阅读量


#### 时间处理

`new Date()` 生成的时间： `addTime: 2017-10-17T03:07:53.115Z,`

时间的处理比较麻烦，所以swig模板引擎为我们提供了相关过滤器；我们可以使用管道符后跟过滤器函数的形式来使用过滤器。

```html
<td>{{content.addTime | date('Y年m月d日 H:i:s')}}</td>
```
`'Y年m月d日 H:i:s'`为格式字符串。  
输出结果为： `2017年10月17日 03:07:53`
而当前电脑显示的本地时间为： 2017年10月17日 11点多
...
`60`：表示60分钟； `8*60`:表示8个小时（我们是东八区）；`-`表示偏移方向。


**查看 date 的源码(由模板引擎swig提供)：**
```
node_modules > swig > lib > filters.js （在该文件中寻找date函数）
```
...
**html页面编写：**    
先将整个页面的内容全部写出，并应用样式；之后再进行逻辑判断哪些内容需要显示。


**ajax:**   
发送ajax时实际也是发送http请求，所以也会携带cookie。

服务器发送给ajax使用的数据直接是json格式即可，可用 `res.json()` 发送该类数据。


**隐藏域的使用：**

```html
<input type="hidden" id="contentId" value="{{data.content.id}}">
```

再ajax中需要内容的id，我们可以再服务端渲染页面时将id传递到客户端，然后
在客户端将数据保存在隐藏域中，然后再ajax就可以通过获取隐藏域中的值来获取到该id值。


错误：   
描述：点击评论处的提交，没有提交任何数据，代码中的Log也都没有打印出（服务端的log和客户端的log都没有打印），还以为是ajax相关的问题。直到后来无意中的操作使其打印出了一个错误提示我 Content 的引用问题，这才发现在 api.js中没有引入 Content 模型。


要想让最新的评论显示在最上边，可以先将保存评论的数组先进行一次反转。


创建一个 responseData 来统一返回的数据格式。

在ajax中日期的格式化，自己创建用于格式化的函数。

由于评论已经全部都传送到了客户端，所以我们也可以在客户端来处理评论的分页显示的问题。


在处理上下一页的时候使用到了委托 delegate 。
但是在这里出现了一个问题，执行该方法后都会导致页面重载，这会重新为 page 赋值，而无法跳转页面。
